...
open($filehandparamwrite,">"."indelible-hyperparams.txt") or die "Error opening the file\n";
print $filehandparamwrite "Replicate LENGTH_LOG_MEAN LENGTH_LOG_STD\n";
...
foreach my  $dir (@dirs)
{
	...
	#Sampling Species_specific parameters and writing them into the SQLite DB.
        ...
        $shape_seqlength=gsl_ran_flat($rng->raw(),6.6845,7.8239)
        $logscale_seqlength=gsl_ran_flat($rng->raw(),0.0,0.3);


	print $filehandparamwrite $sp_counter, " ", $shape_seqlength, " ", $logscale_seqlength, "\n";
